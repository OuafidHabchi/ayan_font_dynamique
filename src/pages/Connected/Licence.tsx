...
interface Book {
  ...
  promotion: boolean;
  newPrix: number;
  prix: number; // Added missing property
  createdAt: string;
  investmentOptions: {
    affiliation: Boolean,
    codePromo: Boolean,
...
export default function Licence() {
  ...
  useEffect(() => {
    const fetchEbooks = async () => {
      try {
        const res = await axios.get(`${AppURL}/api/Collectionebooks/investment/licence`);
    ...
    const fetchFormations = async () => {
      try {
        const res = await axios.get(`${AppURL}/api/StudioFormationRoutes/investment/licence`);
    ...
    <div className="pt-24 bg-blue-950 min-h-screen px-4 sm:px-6 w-full">
      <div className="mx-auto">
        ...
        <Tab.Group selectedIndex={selectedTab} onChange={setSelectedTab}>
          ...
          {language === 'fr'
            ? 'Sélectionnez un eBook ou une formation pour acheter sa licence.'
            : language === 'en'
              ? 'Select an eBook or a training to purchase its license.'
              : 'اختر كتابًا إلكترونيًا أو دورة لشراء الترخيص الخاص بها.'}
        </p>
          ...
          <Tab.Panels>
            ...
            <Tab.Panel>
                ...
                {ebooks.map((book) => (
                  ...
                  >
                    ...
                    <div className="p-3">
                      ...
                      <p className="text-xs mt-1 text-purple-700 font-bold">
              ...
              {selectedBook && (
                <BookLicenceModal book={selectedBook} language={language} onClose={() => setSelectedBook(null)} InvestmentOption ='licence' />
              )}
            </Tab.Panel>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {formations.map((f) => (
                  ...
                  >
                    ...
                    <div className="p-3">
                      <h3 className="font-bold text-sm">{f.titreFormation}</h3>
                      <p className="text-xs text-gray-600">
                        {language === 'fr' ? 'Par' : 'By'} {f.auteur.prenom} {f.auteur.nom}
                      </p>
                      <p className="text-xs mt-1 text-green-600 font-bold">
                        {f.promotion ? `${f.newPrix.toFixed(2)} $` : `${f.investmentOptions.licenceMontant?.toFixed(2)} $`}
                      </p>
                    </div>
                  </div>
                ))}
